...
function SolveNaiveUnconventional(obj::SolverCSD)
    ...
    for n=1:nEnergies
        ...
        K .= K .+ obj.settings.dE*F(obj,K*W',n)*W;
        ...
        L .= L .+ obj.settings.dE*(X'*F(obj,X*L',n))';
        ...
        S .= S .+ obj.settings.dE.*X'*F(obj,X*S*W',n)*W;
        
                
        # update dose
        # obj.dose .+= dE * u[:,1] * obj.csd.SMid[n] ./ obj.density ./( 1 + (n==1||n==nEnergies));
    end
