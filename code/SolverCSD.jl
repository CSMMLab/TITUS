    ...
    if nq == 1
        ...
        for j = 1:ny
            for k = 1:nz
                ...
                psiCPU[idx] = PsiBeam(obj,obj.qReduced[q,:],energy[n],obj.settings.xMid[1],obj.settings.yMid[j],obj.settings.zMid[k],n-1);
                idx = vectorIndex(nx,ny,nx,j,k)
                psiCPU[idx] = PsiBeam(obj,obj.qReduced[q,:],energy[n],obj.settings.xMid[end],obj.settings.yMid[j],obj.settings.zMid[k],n-1);
            end
        end
        for i = 1:nx
            for k = 1:nz
                idx = vectorIndex(nx,ny,i,1,k)
                psiCPU[idx] = PsiBeam(obj,obj.qReduced[q,:],energy[n],obj.settings.xMid[i],obj.settings.yMid[1],obj.settings.zMid[k],n-1);
                idx = vectorIndex(nx,ny,i,ny,k)
                psiCPU[idx] = PsiBeam(obj,obj.qReduced[q,:],energy[n],obj.settings.xMid[i],obj.settings.yMid[end],obj.settings.zMid[k],n-1);
            end
        end
        for i = 1:nx
            for j = 1:ny
                idx = vectorIndex(nx,ny,i,j,1)
                psiCPU[idx] = PsiBeam(obj,obj.qReduced[q,:],energy[n],obj.settings.xMid[i],obj.settings.yMid[j],obj.settings.zMid[1],n-1);
                idx = vectorIndex(nx,ny,i,j,nz)
                psiCPU[idx] = PsiBeam(obj,obj.qReduced[q,:],energy[n],obj.settings.xMid[i],obj.settings.yMid[j],obj.settings.zMid[end],n-1);
            end
        end
    else
        for q = 1:nq
