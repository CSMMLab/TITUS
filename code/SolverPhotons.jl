...
function SolveFirstCollisionSourcePhotons(obj::SolverCSD)
    ...
    u_e = zeros(nx*ny,N);
    uNew_e = deepcopy(u)
    psiNew_γ = deepcopy(psi_γ)
    psiNew_e = deepcopy(psi_e)
    flux_γ = zeros(size(psi_γ))
    flux_e = zeros(size(psi_e))
    ...
    for n=1:nEnergies
        ...
        sigmaS_γ = SigmaAtEnergy(obj.csd,energy[n])#.*sqrt.(obj.gamma); # TODO: change to photon scattering
        sigmaS_e = SigmaAtEnergy(obj.csd,energy[n])#.*sqrt.(obj.gamma); #TODO: add inscattering from photon interactions
        
        #photon equation
        # solve uncollided photon equation
        solveFlux!(obj,psi_γ,flux_γ);

        psi_γ .= psi_γ .- dE*flux_γ;
        
        psiNew_γ .= psi_γ ./ (1+dE*sigmaS_γ[1]);
       
        Dvec = zeros(obj.pn.nTotalEntries)
        for l = 0:obj.pn.N
            for k=-l:l
                i = GlobalIndex( l, k );
                Dvec[i+1] = sigmaS_γ[l+1]
            end
        end

        D = Diagonal(sigmaS_γ[1] .- Dvec);

        # set boundary condition
        #u[1,:] .= BCLeft(obj,n);

        # perform time update
        uTilde_γ = u_γ .- dE * Rhs(obj,u_γ); 
        ...
        for i = 1:nx
            for j = 1:ny
                idx = (i-1)*nx + j
                uNew_γ[idx,:] = (Id .+ dE*D)\(uTilde_γ[idx,:] .+ dE*Diagonal(Dvec)*obj.M*psiNew_γ[i,j,:]);
                uOUnc[idx] = psiNew_γ[i,j,:]'*obj.M[1,:];
            end
        end
        ...
        #electron equation (don't use first collision approach here, as uncollided electrons can still have inscattering from photons)
        # TODO: The photon flux has to be incorporated here somehow
        Dvec_e = zeros(obj.pn.nTotalEntries)
        for l = 0:obj.pn.N
            for k=-l:l
                i = GlobalIndex( l, k );
                Dvec_e[i+1] = sigmaS_e[l+1]
            end
        end

        D_e = Diagonal(sigmaS_e[1] .- Dvec_e);

        # set boundary condition
        #u[1,:] .= BCLeft(obj,n);

        # perform time update
        uTilde_e = u_e .- dE * Rhs(obj,u_e); 

        for j = 1:size(uNew_e,1)
            uNew_e[j,:] = (Id .+ dE*D_e)\uTilde_e[j,:] .+ dE*Diagonal(Dvec_e)*obj.M*uNew_γ[j,:]);
        end
        
        # update dose (only electrons deposit dose (?))
        obj.dose .+= dE * uNew_e[:,1] * obj.csd.SMid[n] ./ obj.densityVec ./( 1 + (n==1||n==nEnergies));
 
        u_e .= uNew_e;
        next!(prog) # update progress bar
    end
    # return end time and solution
    return 0.5*sqrt(obj.gamma[1])*u_e,obj.dose;

end

function SolvePhotonsDLR(obj::SolverCSD)
    # Get rank
    r=obj.settings.r;
