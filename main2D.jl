...
s = Settings(101, 21, 100, 100, "Lattice")
s.σₛξ = 0;
s.σₛη = 0;
...
## DLRA BUG adaptive
maxV = maximum(rhoBUG[2:end-1,2:end-1])
minV = max(1e-7,minimum(4.0*pi*sqrt(2)*rhoBUG[2:end-1,2:end-1]))
idxNeg = findall((rhoBUG.<=0.0))
rhoBUG[idxNeg] .= NaN;
fig = figure("BUG, log, ϑ coarse",figsize=(10,10),dpi=100)
ax = gca()
pcolormesh(X,Y,4.0*pi*sqrt(2)*rhoBUG[2:(end-1),(end-1):-1:2]',norm=matplotlib.colors.LogNorm(vmin=minV, vmax=maxV))
ax.tick_params("both",labelsize=20) 
plt.xlabel("x", fontsize=20)
plt.ylabel("y", fontsize=20)
plt.title(L"$\Phi$, BUG, $\bar \vartheta =$ "*LaTeXString(string(s.ϵ)), fontsize=25)
tight_layout()
show()
savefig("scalar_flux_PN_log_$(s.problem)_nx$(s.NCells)_N$(s.nPN)_augmented.png")
